...
-- 교육과정
DROP TABLE IF EXISTS pms_lect RESTRICT;
...
-- 교육과정
ALTER TABLE pms_lect
  ADD CONSTRAINT PK_pms_lect -- 교육과정 기본키
    PRIMARY KEY (
      lno -- 교육과정번호
    );
...
CREATE TABLE lms_appl (
  ...
  stat INTEGER  NOT NULL DEFAULT 0 COMMENT '상태' -- 상태
)
COMMENT '수강신청';
...
-- 교육과정
ALTER TABLE pms_lect
  ADD CONSTRAINT FK_lms_mgr_TO_pms_lect -- 매니저 -> 교육과정
    FOREIGN KEY (
      mno -- 매니저번호
    )
...
-- 교육과정
ALTER TABLE pms_lect
  ADD CONSTRAINT FK_lms_room_TO_pms_lect -- 교실 -> 교육과정
    FOREIGN KEY (
      rno -- 교실번호
    )
...
-- 수강신청
ALTER TABLE lms_appl
  ADD CONSTRAINT FK_pms_lect_TO_lms_appl -- 교육과정 -> 수강신청
    FOREIGN KEY (
      lno -- 교육과정번호
    )
    REFERENCES pms_lect ( -- 교육과정
      lno -- 교육과정번호
    );
...
-- 교육과정강사
ALTER TABLE lms_tchr_lect
  ADD CONSTRAINT FK_pms_lect_TO_lms_tchr_lect -- 교육과정 -> 교육과정강사
    FOREIGN KEY (
      lno -- 교육과정번호
    )
    REFERENCES pms_lect ( -- 교육과정
